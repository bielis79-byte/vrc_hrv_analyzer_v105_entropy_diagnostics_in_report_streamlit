...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v10.5", layout="wide")
...
PARAM_GROUPS = {
    ...
    "Control Kubios": ["Entropy_lambda", "Entropy_m", "Entropy_r_factor", "Entropy_SD_ms", "Entropy_r_ms", "Entropy_N", "DFA_alpha1_range", "DFA_alpha2_range", "RQA_threshold_SD", "RQA_emb_dim"],
}
...
def _sample_entropy_core(x, m=2, r=None, zero_policy="nan"):
    ...
    zero_policy:
    - "nan": si A=0 devuelve NaN, evita valores artificialmente inflados.
    - "half_count": compatibilidad anterior, usa A=0.5.
    """
...
def sample_entropy_common(rr_entropy, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None):
    """
    ...
    return _sample_entropy_core(x, m=m, r=r, zero_policy="nan")
...
def mse_common(rr_entropy, scales=KUBIOS_MSE_MAX_SCALE, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None):
    """
    MSE v10.3.

    Cambios:
    - Mantiene la misma entrada λ=500 que SampEn.
    - MSE1 = SampEn.
    - Elimina el pseudoconteo automático A=0.5 de la v10.2, porque inflaba
      escalas altas como MSE8, MSE13 o MSE14.
    - Si A=0 en una escala, devuelve NaN en vez de inventar un valor alto.
    """
    x = np.asarray(rr_entropy, dtype=float)
    x = x[np.isfinite(x)]
    ...
    for scale in range(1, int(scales) + 1):
        ...
        if len(cg) <= m + 2:
            ...
            out[f"MSE{scale}"] = _sample_entropy_core(cg, m=m, r=r, zero_policy="nan")

    out["MSE1"] = _sample_entropy_core(x, m=m, r=r, zero_policy="nan")
    return out
...
def mse_metrics(rr, scales=20, max_scale=None, m=2, r=0.2):
    ...
    return mse_common(rr, scales=scales, m=m, r_factor=r, r_reference=rr)
...
def sample_entropy_diagnostic_rows(rr_entropy, scales=20, m=2, r_factor=0.2, r_reference=None):
    ...
    Muestra por cada escala:
    - N de la serie coarse-grained
    - SD de la escala
    - r usado
    - B matches para m
    - A matches para m+1
    ...
    Sirve para detectar por qué Kubios calcula una escala y la app devuelve NaN.
    """
    x = np.asarray(rr_entropy, dtype=float)
    x = x[np.isfinite(x)]
...
def entropy_diagnostic_figure(diag_df):
    """
    Figura diagnóstica MSE: SampEn por escala + marcadores de A/B.
    """
    fig = go.Figure()
    ...
    fig.add_trace(go.Bar(
        x=diag_df["Escala"],
        y=diag_df["SampEn_app"],
        name="SampEn/MSE app",
        marker=dict(color="#1f77b4"),
        opacity=0.65,
        hovertemplate="Escala %{x}<br>SampEn=%{y:.4f}<extra></extra>",
    ))

    # Marcar escalas con A=0 o B=0
    ...
    if not bad.empty:
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=10, symbol="x", color="#ff4b4b"),
            hovertemplate="Escala %{x}<br>%{text}<extra></extra>",
        ))

    fig.update_layout(
        title="Diagnóstico SampEn / MSE: valores calculados y escalas con A/B insuficiente",
...
def _single_record_report(record_name, metrics_df, windows, rr=None):
    ...
    if any(c in metrics_df.columns for c in MSE_COLUMNS):
        ...
        if rr is not None:
            ...
            lines.append("")
            lines.append("Este bloque permite comprobar por qué algunas escalas MSE no coinciden con Kubios. Muestra el número de puntos por escala, la tolerancia usada y los conteos B/A de SampEn.")
            lines.append("")
            lines.append("| Fase | Escala | N | r ms | B matches | A matches | A/B | SampEn app | Estado |")
            lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---|")
                ...
                try:
                    ...
                    for _, drow in diag_df.iterrows():
                        lines.append(
                            ...
                            f"{_fmt_num(drow.get('SampEn_app'))} | "
                            f"{drow.get('Estado','')} |"
                        )
...
st.title("VRC / HRV RRi Analyzer Pro v10.5")
...
def free_pairs_hvg_metrics_table(long_df, pairs):
    ...
    st.caption(
        "Muestra por escala MSE: N, SD, r=0.2×SD, B matches, A matches y el motivo si una escala no se calcula. "
        "Útil para comparar con Kubios cuando MSE7-MSE20 difieren."
    )
